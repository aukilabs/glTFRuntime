...
struct FglTFRuntimeSkeletalMeshContextFinalizer
{
	...
	FglTFRuntimeSkeletalMeshContextFinalizer(TSharedRef<FglTFRuntimeSkeletalMeshContext, ESPMode::ThreadSafe> InSkeletalMeshContext,
...
USkeletalMesh* FglTFRuntimeParser::CreateSkeletalMeshFromLODs(TSharedRef<FglTFRuntimeSkeletalMeshContext, ESPMode::ThreadSafe> SkeletalMeshContext)
{
	...
	if (SkeletalMeshContext->SkeletalMeshConfig.SkeletonConfig.bClearRotations || SkeletalMeshContext->SkeletalMeshConfig.SkeletonConfig.
		CopyRotationsFrom)
	{
						...
						for (int32 JointPartIndex = 0; JointPartIndex < 4; JointPartIndex++)
						{
							...
							else if (!SkeletalMeshContext->SkeletalMeshConfig.bIgnoreMissingBones)
							{
								AddError("LoadSkeletalMesh_Internal()",
								         FString::Printf(TEXT("Unable to find map for bone %u"), Joints[JointPartIndex]));
								return nullptr;
							}
						}
					}
...
USkeletalMesh* FglTFRuntimeParser::FinalizeSkeletalMeshWithLODs(TSharedRef<FglTFRuntimeSkeletalMeshContext, ESPMode::ThreadSafe> SkeletalMeshContext)
{
	...
	for (int32 LODIndex = 0; LODIndex < SkeletalMeshContext->LODs.Num(); LODIndex++)
	{
		...
		for (int32 MatIndex = 0; MatIndex < SkeletalMeshContext->LODs[LODIndex].Primitives.Num(); MatIndex++)
		{
			...
			SkeletalMaterials[NewMatIndex].MaterialSlotName = FName(FString::Printf(
				TEXT("LOD_%d_Section_%d_%s"), LODIndex, MatIndex, *(SkeletalMeshContext->LODs[LODIndex].Primitives[MatIndex].MaterialName)));
		}
#if WITH_EDITOR
		IMeshBuilderModule& MeshBuilderModule = IMeshBuilderModule::GetForRunningPlatform();
#if ENGINE_MAJOR_VERSION == 4 && ENGINE_MINOR_VERSION >= 27
		FSkeletalMeshBuildParameters SkeletalMeshBuildParameters(SkeletalMeshContext->SkeletalMesh,
		                                                         GetTargetPlatformManagerRef().GetRunningTargetPlatform(), LODIndex, false);
	...
	{
		if (CanReadFromCache(SkeletalMeshContext->SkeletalMeshConfig.SkeletonConfig.CacheMode) && SkeletonsCache.Contains(
			SkeletalMeshContext->SkinIndex))
		{
	...
	if (!SkeletalMeshContext->SkeletalMeshConfig.SaveToPackage.IsEmpty())
	{
		...
		{
			const FString Filename = FPackageName::LongPackageNameToFilename(SkeletalMeshContext->SkeletalMeshConfig.SaveToPackage,
...
void FglTFRuntimeParser::LoadSkeletalMeshAsync(const int32 MeshIndex, const int32 SkinIndex, FglTFRuntimeSkeletalMeshAsync AsyncCallback,
                                               const FglTFRuntimeSkeletalMeshConfig& SkeletalMeshConfig)
{
	TSharedRef<FglTFRuntimeSkeletalMeshContext, ESPMode::ThreadSafe> SkeletalMeshContext = MakeShared<
		FglTFRuntimeSkeletalMeshContext, ESPMode::ThreadSafe>(AsShared(), SkeletalMeshConfig);
...
USkeletalMesh* FglTFRuntimeParser::LoadSkeletalMeshRecursive(const FString& NodeName, const int32 SkinIndex, const TArray<FString>& ExcludeNodes,
                                                             const FglTFRuntimeSkeletalMeshConfig& SkeletalMeshConfig)
{
...
void FglTFRuntimeParser::LoadSkeletalMeshRecursiveAsync(const FString& NodeName, const int32 SkinIndex, const TArray<FString>& ExcludeNodes,
                                                        FglTFRuntimeSkeletalMeshAsync AsyncCallback,
                                                        const FglTFRuntimeSkeletalMeshConfig& SkeletalMeshConfig)
{
	TSharedRef<FglTFRuntimeSkeletalMeshContext, ESPMode::ThreadSafe> SkeletalMeshContext = MakeShared<
                                                             ...
                                                             const FglTFRuntimeSkeletalAnimationConfig& SkeletalAnimationConfig)
{
	...
	{
		AddError("LoadNodeSkeletalAnimation()", "Cannot load joints for Node Index " + FString::FromInt(NodeIndex));
		return nullptr;
	}
	...
	for (int32 JsonAnimationIndex = 0; JsonAnimationIndex < JsonAnimations->Num(); JsonAnimationIndex++)
	{
		...
		FSkeletalAnimationRawData AnimationRawData;
		bool bAnimationFound = false;
		if (!LoadSkeletalAnimation_Internal(JsonAnimationObject.ToSharedRef(), AnimationRawData, SkeletalAnimationConfig,
		...
		if (bAnimationFound)
		{
			return ComposeAnimSequenceFromTracks(SkeletalMesh, SkeletalAnimationConfig, AnimationRawData);
		}
	}
...
bool FglTFRuntimeParser::GetNodesSkinJoints(const int32 NodeIndex, TArray<int32>& OutJoints)
{
	FglTFRuntimeNode Node;
	...
	{
		AddError("GetNodesSkinJoints()", FString::Printf(TEXT("No skin defined for node %d"), NodeIndex));
		return false;
	}

	const TSharedPtr<FJsonObject> JsonSkinObject = GetJsonObjectFromRootIndex("skins", Node.SkinIndex);
	if (!JsonSkinObject)
	{
		AddError("GetNodesSkinJoints()", "No skins defined in the asset");
		return false;
	}

	const TArray<TSharedPtr<FJsonValue>>* JsonJoints;
	if (!JsonSkinObject->TryGetArrayField("joints", JsonJoints) || !JsonJoints->Num())
	{
		AddError("GetNodesSkinJoints()", "No joints defined in the skin");
		return false;
	}

	for (const TSharedPtr<FJsonValue> JsonJoint : (*JsonJoints))
	{
		int64 JointIndex;
		if (!JsonJoint->TryGetNumber(JointIndex))
		{
			AddError("GetNodesSkinJoints()", "Joints json field contains non-number indices.");
			return false;
		}
		OutJoints.Add(JointIndex);
	}
...
bool FglTFRuntimeParser::FillAnimSequenceFromTracks(UAnimSequence* AnimSequence, USkeletalMesh* SkeletalMesh,
	const FglTFRuntimeSkeletalAnimationConfig& SkeletalAnimationConfig, FSkeletalAnimationRawData& AnimationRawData)
{
	int32 NumFrames = AnimationRawData.Duration * 30;
	...
	AnimSequence->SequenceLength = AnimationRawData.Duration;
	...
	for (TPair<FString, FRawAnimSequenceTrack>& Pair : AnimationRawData.Tracks)
	{
	...
	for (TPair<FName, TArray<TPair<float, float>>>& Pair : AnimationRawData.MorphTargetCurves)
	{
		...
		FFloatCurve* NewCurve = static_cast<FFloatCurve*>(AnimSequence->RawCurveData.GetCurveData(SmartName.UID, ERawCurveTrackTypes::RCT_Float));
	...
	if (!bHasTracks)
	{
		...
		if (!IsInGameThread())
		{
			AsyncTask(ENamedThreads::GameThread, [AnimSequence]
			{
...
UAnimSequence* FglTFRuntimeParser::ComposeAnimSequenceFromTracks(USkeletalMesh* SkeletalMesh,
                                                                 const FglTFRuntimeSkeletalAnimationConfig& SkeletalAnimationConfig,
                                                                 FSkeletalAnimationRawData& AnimationRawData)
{
	// Compose full name in the format of "AnimationName_MeshName" (if the Optional name is set).
	FName AnimFullName = AnimationRawData.OptionalName.IsEmpty() ? NAME_None : FName(AnimationRawData.OptionalName + "_" + SkeletalMesh->GetName());
	UAnimSequence* AnimSequence = NewObject<UAnimSequence>(GetTransientPackage(), AnimFullName, RF_Public);

	if (FillAnimSequenceFromTracks(AnimSequence, SkeletalMesh, SkeletalAnimationConfig, AnimationRawData))
	{
		return AnimSequence;
	}
	AnimSequence->ConditionalBeginDestroy();
	return nullptr;
}
                                                         ...
                                                         const FglTFRuntimeSkeletalAnimationConfig& SkeletalAnimationConfig)
{
	...
	FSkeletalAnimationRawData AnimationRawData;
	if (!LoadSkeletalAnimation_Internal(JsonAnimationObject.ToSharedRef(), AnimationRawData, SkeletalAnimationConfig,
	                                    [](const FglTFRuntimeNode& Node) -> bool { return true; }))
	{
		return nullptr;
	}
...
TFuture<bool> FglTFRuntimeParser::LoadSkeletalAnimation_Internal_OnThreadPool(const int32 AnimationIndex, FSkeletalAnimationRawData& OutAnimationData,
	const FglTFRuntimeSkeletalAnimationConfig& SkeletalAnimationConfig, TFunctionRef<bool(const FglTFRuntimeNode& Node)> Filter)
{
	// Run on the thread pool and return a future to wait for result.
	return Async(EAsyncExecution::ThreadPool, [&]()
	{
		TSharedPtr<FJsonObject> JsonAnimationObject = GetJsonObjectFromRootIndex("animations", AnimationIndex);
		return LoadSkeletalAnimation_Internal(JsonAnimationObject.ToSharedRef(), OutAnimationData, SkeletalAnimationConfig, Filter);
	});
}
	
bool FglTFRuntimeParser::LoadSkeletalAnimation_Internal(TSharedRef<FJsonObject> JsonAnimationObject, FSkeletalAnimationRawData& OutAnimationData, const FglTFRuntimeSkeletalAnimationConfig& SkeletalAnimationConfig,
                                                        TFunctionRef<bool(const FglTFRuntimeNode& Node)> Filter)
{
	// Try getting animation name first.
	JsonAnimationObject->TryGetStringField("name", OutAnimationData.OptionalName);

	auto Callback = [&](const FglTFRuntimeNode& Node, const FString& Path, const TArray<float> Timeline, const TArray<FVector4> Values)
	{
		int32 NumFrames = OutAnimationData.Duration * 30;
		...
		if (Path == "rotation" && !SkeletalAnimationConfig.bRemoveRotations)
		{
			if (Timeline.Num() != Values.Num())
			{
				AddError("LoadSkeletalAnimation_Internal()",
				         FString::Printf(
					         TEXT("Animation input/output mismatch (%d/%d) for rotation on node %d"), Timeline.Num(), Values.Num(), Node.Index));
				return;
			}

			if (!OutAnimationData.Tracks.Contains(Node.Name))
			{
				OutAnimationData.Tracks.Add(Node.Name, FRawAnimSequenceTrack());
			}

			FRawAnimSequenceTrack& Track = OutAnimationData.Tracks[Node.Name];
		...
		else if (Path == "translation" && !SkeletalAnimationConfig.bRemoveTranslations)
		{
			if (Timeline.Num() != Values.Num())
			{
				AddError("LoadSkeletalAnimation_Internal()",
				         FString::Printf(
					         TEXT("Animation input/output mismatch (%d/%d) for translation on node %d"), Timeline.Num(), Values.Num(), Node.Index));
				return;
			}
		...
		else if (Path == "scale" && !SkeletalAnimationConfig.bRemoveScales)
		{
			if (Timeline.Num() != Values.Num())
			{
				AddError("LoadSkeletalAnimation_Internal()",
				         FString::Printf(
					         TEXT("Animation input/output mismatch (%d/%d) for scale on node %d"), Timeline.Num(), Values.Num(), Node.Index));
				return;
			}
		...
		else if (Path == "weights" && !SkeletalAnimationConfig.bRemoveMorphTargets)
		{
			...
			if (Timeline.Num() != Values.Num() / MorphTargetNames.Num())
			{
				AddError("LoadSkeletalAnimation_Internal()",
				         FString::Printf(
					         TEXT("Animation input/output mismatch (%d/%d) for weights on node %d"), Timeline.Num(), Values.Num(), Node.Index));
				return;
			}
			...
			for (int32 MorphTargetIndex = 0; MorphTargetIndex < MorphTargetNames.Num(); MorphTargetIndex++)
			{
				...
				for (int32 TimelineIndex = 0; TimelineIndex < Timeline.Num(); TimelineIndex++)
				{
					TPair<float, float> Curve = TPair<float, float>(Timeline[TimelineIndex],
					                                                Values[TimelineIndex * MorphTargetNames.Num() + MorphTargetIndex].X);
					Curves.Add(Curve);
				}
				OutAnimationData.MorphTargetCurves.Add(MorphTargetName, Curves);
			}
		}
	};

	FString IgnoredName;
	return LoadAnimation_Internal(JsonAnimationObject, OutAnimationData.Duration, IgnoredName, Callback, Filter);
}
